...
void forward_network(network net)
{
    ...
    for(i = 0; i < net.n; ++i){//从输入层到输出层走一趟
        ...
        l.forward(l, net);
        net.input = l.output;//将本层输出作为赋值为下一层的输入
        if(l.truth) {
            net.truth = l.output;
        }
    }
    calc_network_cost(net);//计算代价函数的值
}
...
{
    assert(d.X.rows % net.batch == 0);//d.X.rows保存的是单个gpu本次iteration所需的图像数量
    int batch = net.batch;
    int n = d.X.rows / batch;//分为多少次小iteration

    int i;
    float sum = 0;
    for(i = 0; i < n; ++i){
        get_next_batch(d, batch, i*batch, net.input, net.truth);//将训练数据和truth数据从d拷贝到net
        float err = train_network_datum(net);
        sum += err;
    }
    return (float)sum/(n*batch);
...
{
#ifdef GPU
    if(gpu_index >= 0)  return network_predict_gpu(net, input);//使用gpu时跳转network_kernel.cu
#endif
    net.input = input;
    net.truth = 0;
    net.train = 0;
