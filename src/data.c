...
void fill_truth_detection(char *path, int num_boxes, float *truth, int classes, int flip, float dx, float dy, float sx, float sy)
{
    ...
    find_replace(labelpath, ".JPEG", ".txt", labelpath);
    int count = 0;
    box_label *boxes = read_boxes(labelpath, &count);//读取labels的过程，boxes[i].id记录下txt文件中的label序号
    randomize_boxes(boxes, count);
    correct_boxes(boxes, count, dx, dy, sx, sy, flip);
    ...
    for (i = 0; i < count; ++i) {
        ...
        if ((w < .001 || h < .001)) continue;//忽略小于图像尺寸0.001倍的bboxing

        truth[i*5+0] = x;//这个truth指针就是对应的net.data.X.val[i]
        truth[i*5+1] = y;
        truth[i*5+2] = w;
        truth[i*5+3] = h;
        truth[i*5+4] = id;
...
{
    char **random_paths = get_random_paths(paths, n, m);//在整个数据集中随机取出此次iteration所用的n张图像
    int i;
    data d = {0};
    d.shallow = 0;

    d.X.rows = n;//分配保存图像数据的矩阵大小，每行保存一张图像的信息，共有n行
    d.X.vals = calloc(d.X.rows, sizeof(float*));
    d.X.cols = h*w*3;
    ...
    for(i = 0; i < n; ++i){
        ...
        int flip = rand()%2;
        if(flip) flip_image(sized);
        d.X.vals[i] = sized.data;//d.X.vals存放经过改变后的图像自我生层的val，跟标记文档txt内容无关


        fill_truth_detection(random_paths[i], boxes, d.y.vals[i], classes, flip, -dx/w, -dy/h, nw/w, nh/h);//处理一张图像,由图像路径找到对应的txt标记文件,d.y.vals将保存标记类别和bboxing
...
void *load_threads(void *ptr)
{
    ...
    data *out = args.d;//存放数据的地址
    int total = args.n;//该次iteration需要的图像数量
    free(ptr);
    data *buffers = calloc(args.threads, sizeof(data));//YOLO定义为8个线程，8个data结构的大小
    pthread_t *threads = calloc(args.threads, sizeof(pthread_t));//8个线程同时加载图像数据，加快速度
    ...
    for(i = 0; i < args.threads; ++i){
        pthread_join(threads[i], 0);//等待所有数据加载线程的结束
    }
...
matrix concat_matrix(matrix m1, matrix m2)
{
    ...
    matrix m;
    m.cols = m1.cols;
    m.rows = m1.rows+m2.rows;//等于图像数量
    m.vals = calloc(m1.rows + m2.rows, sizeof(float*));
    for(i = 0; i < m1.rows; ++i){
        m.vals[count++] = m1.vals[i];//复制物体类比序号和bboxing
    }
    for(i = 0; i < m2.rows; ++i){
