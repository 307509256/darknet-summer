...
{
    int i;
    for(i = 0; i < net.n; ++i){//net.n为神经网络深度,开始逐层训练
        net.index = i;
        layer l = net.layers[i];
        if(l.delta_gpu){
            fill_gpu(l.outputs * l.batch, 0, l.delta_gpu, 1);
        }
        l.forward_gpu(l, net);//LOOK HERE
        //可以在这里通过l.output读取到该层的输出feature map
        net.input_gpu = l.output_gpu;//将该层输出设置为下一层的输入
        net.input = l.output;
        if(l.truth) {
...
{
    cuda_set_device(net.gpu_index);
    cuda_push_array(net.input_gpu, input, net.inputs*net.batch);//将数据转为net成员变量
    net.truth = 0;
    net.train = 0;
    forward_network_gpu(net);
    return net.output;
