...
void parse_net_options(list *options, network *net)//将参数提取并配置为net的成员变量
{
    ...
    net->batch /= subdivs;//实际的batch size是原值切分后的大小,跟time_steps也相关
    ...
    net->inputs = option_find_int_quiet(options, "inputs", net->h * net->w * net->c);//输入的像素点总数
    ...
    char *policy_s = option_find_str(options, "policy", "constant");//本函数下面的代码都是在配置policy
...
{
    list *sections = read_cfg(filename);//返回一个链表，第一个链表节点存储net的训练参数，之后的每个链表节点都是一个层结构，层结构里面包含着一个描述该层参数的内部链表。
    node *n = sections->front;//首节点，保存着神经网络的训练参数
    if(!n) error("Config file has no sections");
    network net = make_network(sections->size - 1);//这个函数只分配了相应的内存，并没有加载构建网络层，第一个节点不是层定义结构，减去1
    net.gpu_index = gpu_index;
    size_params params;

    section *s = (section *)n->val;
    list *options = s->options;//[net]神经网络配置参数链表
    if(!is_network(s)) error("First section must be [net] or [network]");
    parse_net_options(options, &net);//真正配置net参数的函数

    params.h = net.h;
    params.w = net.w;
    params.c = net.c;
    params.inputs = net.inputs;// net.h*net.w*net.c
    params.batch = net.batch;
    params.time_steps = net.time_steps;
    params.net = net;

    size_t workspace_size = 0;
    n = n->next;//从第一层开始，构造神经网络的各个层
    int count = 0;
    free_section(s);
    ...
    while(n){
        ...
        net.layers[count] = l;//将构建好的层添加到net
    ...
    net.input = calloc(net.inputs*net.batch, sizeof(float));//先分配内存，等待输入层载入数据
    ...
    while((line=fgetl(file)) != 0){
        ...
        switch(line[0]){
            case '['://一个层定义
                ...
                current->options = make_list();//层结构内部用来保存参数的链表
                current->type = line;//层的类型，带[]
                break;
            case '\0':
            case '#':
            case ';':
                ...
                break;
            default:
                if(!read_option(line, current->options)){//这个函数会将bn/padding/size/stride/activation/fielter参数保存到对应层结构的内部链表。
